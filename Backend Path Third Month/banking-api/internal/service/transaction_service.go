...
func NewTransactionService(
	...
	return &TransactionService{
		txs: txs, users: users, balances: balances, audit: audit, pool: pool,
...
func (s *TransactionService) Credit(ctx context.Context, req domain.CreditRequest) (*domain.Transaction, error) {
	...
	tx := &domain.Transaction{ToUserID: req.UserID, Amount: req.Amount, Type: domain.TypeCredit, Status: domain.StatusPending}
...
func (s *TransactionService) Debit(ctx context.Context, req domain.DebitRequest) (*domain.Transaction, error) {
	...
	from := req.UserID
	tx := &domain.Transaction{FromUserID: &from, ToUserID: req.UserID, Amount: req.Amount, Type: domain.TypeDebit, Status: domain.StatusPending}
	if err := tx.Validate(); err != nil {
		return nil, err
	}
	return s.execute(ctx, tx, func() error {
...
func (s *TransactionService) Transfer(ctx context.Context, req domain.TransferRequest) (*domain.Transaction, error) {
	...
	from := req.FromUserID
	tx := &domain.Transaction{FromUserID: &from, ToUserID: req.ToUserID, Amount: req.Amount, Type: domain.TypeTransfer, Status: domain.StatusPending}
	if err := tx.Validate(); err != nil {
		return nil, err
	}
	return s.execute(ctx, tx, func() error {
...
func (s *TransactionService) execute(ctx context.Context, tx *domain.Transaction, apply func() error) (*domain.Transaction, error) {
	...
	s.pool.Submit(domain.TransactionJob{Transaction: tx, Apply: apply, ResultCh: resultCh})
	...
	case err := <-resultCh:
		...
		_ = s.audit.Create(ctx, &domain.AuditLog{
			EntityType: "transaction", EntityID: tx.ID, Action: string(tx.Type), Details: mustJSON(tx),
		})
		return tx, nil
	case <-ctx.Done():
		return nil, ctx.Err()
...
func (s *TransactionService) Rollback(ctx context.Context, id int64) error {
	...
	tx.MarkRolledBack()
	_ = s.txs.UpdateStatus(ctx, tx.ID, domain.StatusRolledBack)
	_ = s.audit.Create(ctx, &domain.AuditLog{
		EntityType: "transaction", EntityID: tx.ID, Action: "rollback",
